...
export class CTCDecoder {
  ...
  /**
   * CTC prefix beam search with Quran trie constraints.
   * Only allows outputs that are valid prefixes of Quran verses.
   *
   * @param logprobs CTC log probabilities [timeSteps x vocabSize]
   * @param timeSteps Number of time steps
   * @param vocabSize Vocabulary size
   * @param trie QuranTrie instance for constraining paths
   * @param opts Beam search options
   * @returns Top hypotheses that are valid Quran prefixes or complete verses
   */
  constrainedBeamSearch(
    logprobs: Float32Array,
    timeSteps: number,
    vocabSize: number,
    trie: { getNode: (tokenIDs: number[]) => { children: Map<number, any>; isVerseEnd: boolean; verseRefs: Array<{ surah: number; ayah: number }> } | null },
    opts: BeamSearchOptions = {},
  ): Hypothesis[] {
    const beamWidth = opts.beamWidth ?? 10;
    const topK = opts.topK ?? 20;
    ...
    for (let t = 0; t < timeSteps; t++) {
      ...
      for (const [key, beam] of beams) {
        ...
        // --- Case 1: Emit blank ---
        // Blank always allowed (doesn't advance trie position)
        const blankProb = beamTotal + blankLogProb;
        if (newBeams.has(key)) {
          const existing = newBeams.get(key)!;
          existing.probBlank = logAdd(existing.probBlank, blankProb);
        ...
        // --- Case 2: Emit non-blank tokens (constrained by trie) ---
        // Get valid next tokens from trie
        const trieNode = trie.getNode(beam.tokens);
        if (!trieNode) continue; // Invalid prefix, skip extensions

        const validTokens = Array.from(trieNode.children.keys());

        // Only extend with tokens that are both:
        // 1. In top-K for this frame
        // 2. Valid children in the trie
        for (const [tokenId, tokenLogProb] of topTokens) {
          if (!validTokens.includes(tokenId)) continue; // Trie constraint

...
Requirements:
    pip install transformers[torch] peft datasets evaluate jiwer \
               accelerate soundfile librosa torchaudio
"""
...
def load_and_prepare_datasets(
    ...
    try:
        ...
        logger.info("Attempting to load with streaming and convert...")
        everyayah = load_dataset(EVERYAYAH_DATASET, streaming=False, trust_remote_code=True)

    # Try loading RetaSy for diverse reciter coverage
    retasy = None
    try:
    ...
    def detect_columns(dataset):
        ...
        sample_cols = list(dataset.column_names.values())[0] if isinstance(
    ...
    # Build unified train/val splits
    train_datasets = []
    val_datasets = []

    # EveryAyah splits
    if "train" in everyayah:
        train_datasets.append(("everyayah", everyayah["train"], ea_audio_col, ea_text_col))
    if "validation" in everyayah:
        val_datasets.append(("everyayah", everyayah["validation"], ea_audio_col, ea_text_col))
    elif "test" in everyayah:
        val_datasets.append(("everyayah", everyayah["test"], ea_audio_col, ea_text_col))

    # RetaSy splits (if available)
    if retasy is not None:
        ...
        if "train" in retasy:
            train_datasets.append(("retasy", retasy["train"], rt_audio_col, rt_text_col))
    ...
    def make_preprocess_fn(audio_col: str, text_col: str, augment: bool = False):
        """Create a preprocessing function for map()."""

        def preprocess(batch):
            # Extract audio arrays and sample rates
            ...
            for audio_item, text in zip(audios, texts):
                ...
                if augment:
                    ...
                    # Noise injection (50% probability to preserve some clean samples)
    ...
    processed_train = []
    for name, ds, audio_col, text_col in train_datasets:
    ...
    processed_val = []
    for name, ds, audio_col, text_col in val_datasets:
    ...
    train_dataset = concatenate_datasets(processed_train) if processed_train else None
    val_dataset = concatenate_datasets(processed_val) if processed_val else None

    if train_dataset is None:
        raise RuntimeError("No training data loaded. Check dataset availability.")

    logger.info(
        "Final dataset sizes: train=%d, val=%s",
        len(train_dataset),
...
def main():
    ...
    dataset = load_and_prepare_datasets(processor)
